...
from workflow_use.llm import get_llm, DEFAULT_LLM_MODEL
...
def auto_load_env_inputs(input_definitions: list) -> dict:
	"""
	Automatically load workflow inputs from environment variables.
	
	Checks if an environment variable exists matching the input name (case-insensitive).
	Returns a dict of {input_name: env_value} for any matches found.
	"""
	inputs = {}
	for input_def in input_definitions:
		input_name = input_def.name
		# Check for exact match first, then uppercase version
		env_value = os.getenv(input_name) or os.getenv(input_name.upper())
		if env_value:
			inputs[input_name] = env_value
	return inputs
...
def run_workflow_no_ai_command(
	...
	async def _run_workflow_no_ai():
		...
		try:
			...
			browser = Browser(use_cloud=use_cloud)
			# Create LLM instance for fallback agent and extraction
			dummy_llm = None
			extraction_llm = None
...
def generate_workflow_from_task(
	...
	use_cloud: bool = typer.Option(False, help='Use Browser-Use Cloud browser'),
	with_credentials: bool = typer.Option(False, '--with-credentials', '-c', help='Include credentials from .env for actual login (stored as variables in workflow)'),
):
	"""
	...
	Example:
	  ...
	  python cli.py generate-workflow "Login to PNC and get balances" --with-credentials
	"""
	...
	# Build enhanced task with credentials if requested
	enhanced_task = task
	sensitive_data = {}
	if with_credentials:
		# Collect all credential-like env vars (containing EMAIL, PASSWORD, USER, etc.)
		credential_keywords = ['EMAIL', 'PASSWORD', 'USER', 'USERNAME', 'LOGIN', 'SECRET', 'OTP']
		credential_names = []
		
		for key, value in os.environ.items():
			if any(kw in key.upper() for kw in credential_keywords) and value:
				# Store actual values in sensitive_data dict
				sensitive_data[key] = value
				credential_names.append(key)
		
		if credential_names:
			typer.secho(f'🔐 Loaded {len(credential_names)} credentials from .env', fg=typer.colors.BLUE)
			
			# Don't include actual values in the prompt - just tell the agent what's available
			# The actual values are passed via sensitive_data parameter
			enhanced_task = f"""{task}

AVAILABLE CREDENTIALS (use these environment variable names):
{chr(10).join(credential_names)}

Look up the values from the environment and use the credentials that match the service you're logging into."""
		else:
			typer.secho('⚠️  No credentials found in .env', fg=typer.colors.YELLOW)
	...
	try:
		...
		workflow_definition = asyncio.run(
			healing_service.generate_workflow_from_prompt(
				...
				use_cloud=use_cloud,
				sensitive_data=sensitive_data if with_credentials else None,
			)
		)
	...
	try:
		if prompt:
			...
			if all_required_have_defaults:
				...
				# Has required inputs without defaults - try to load from environment variables
				env_inputs = auto_load_env_inputs(workflow_definition.input_schema)
				required_inputs = [inp for inp in workflow_definition.input_schema if inp.required]
				missing_required = [inp.name for inp in required_inputs if inp.name not in env_inputs]
				
				if not missing_required:
					# All required inputs found in environment variables
					typer.secho('🔐 Auto-loaded credentials from environment:', fg=typer.colors.GREEN)
					typer.echo()
					
					# Build full inputs dict with env values and defaults
					inputs = {}
					for inp in workflow_definition.input_schema:
						if inp.name in env_inputs:
							inputs[inp.name] = env_inputs[inp.name]
							# Mask sensitive values
							display_val = '*****' if 'password' in inp.name.lower() or 'secret' in inp.name.lower() else env_inputs[inp.name]
							typer.echo(f'  • {inp.name} = {typer.style(display_val, fg=typer.colors.BLUE)} (from env)')
						else:
							default_value = getattr(inp, 'default', None)
							if default_value is not None:
								inputs[inp.name] = default_value
								typer.echo(f'  • {inp.name} = {typer.style(str(default_value), fg=typer.colors.CYAN)} (default)')
					typer.echo()
					
					typer.secho('▶️  Running workflow...', fg=typer.colors.CYAN)
					workflow = Workflow.load_from_file(
						temp_file, llm_instance, page_extraction_llm=page_extraction_llm, use_cloud=use_cloud
					)
					result = asyncio.run(workflow.run(inputs=inputs))
					...
					# Some required inputs missing - show help
					...
					for inp in workflow_definition.input_schema:
						...
						env_hint = f' ✅ found in env' if inp.name in env_inputs else f' ⚠️  set {inp.name} env var'
						typer.echo(f'  • {inp.name} ({inp.type}, {required}{default_str}){env_hint}')
					typer.echo()
					typer.echo('Options:')
					typer.echo(f'  1. Set environment variables: {", ".join(missing_required)}')
					typer.echo(f'  2. Run as tool: python cli.py run-stored-workflow {workflow_id} --prompt "Your task"')
					typer.echo(f'  3. Run with inputs: python cli.py run-workflow {metadata.file_path if metadata else temp_file}')
