...
**IMPORTANT: Output ONLY valid JSON. Do NOT wrap your response in markdown code blocks (no \`\`\`json or \`\`\`). Return raw JSON only.**
...
#### EXACT Step Type Values (CRITICAL - use these EXACT strings):

| Step Type | `type` value (EXACT) | Required Fields |
|-----------|---------------------|-----------------|
| Navigate to URL | `"navigation"` | `url` |
| Click element | `"click"` | `target_text` OR `elementHash` |
| Input text | `"input"` | `value` (REQUIRED!), and (`target_text` OR `elementHash`) |
| Select dropdown | `"select_change"` | `selectedText`, and (`target_text` OR `elementHash`) |
| Press key | `"key_press"` | `key`, and (`target_text` OR `elementHash`) |
| Scroll page | `"scroll"` | `scrollX`, `scrollY` |
| Go back | `"go_back"` | (none) |
| Go forward | `"go_forward"` | (none) |
| Extract content | `"extract_page_content"` | `goal` |
| Agent task | `"agent"` | `task` |

**⚠️ CRITICAL**: Use the EXACT type values above. Do NOT use `"navigate"` (use `"navigation"`), do NOT use `"keypress"` (use `"key_press"`).

#### 🚨 REQUIRED FIELDS FOR INPUT STEPS (CRITICAL!) 🚨

**Input steps MUST have BOTH:**
1. `"value"`: The text to enter (can be a variable like `"{{username}}"` or literal text)
2. `"target_text"` OR `"elementHash"`: Where to enter it

**CORRECT input step example:**
```json
{
  "type": "input",
  "target_text": "User ID",
  "value": "{{pnc_user_id}}",
  "description": "Enter the PNC user ID"
}
```

**WRONG (missing value field - WILL FAIL!):**
```json
{
  "type": "input",
  "target_text": "User ID",
...
- Use the action types listed in the table above
- The `"type"` field must match EXACTLY one of the values in the table
- Include all required parameters as specified
