...
class HealingService:
	...
	async def create_workflow_definition(
		...
		# Use string replacement instead of .format() to avoid conflicts with {variable} syntax in prompt
		prompt_content = prompt_content.replace('{goal}', task)
		prompt_content = prompt_content.replace('{actions}', BuilderService._get_available_actions_markdown())
		...
		# Some models via OpenRouter may wrap JSON in markdown code blocks, so we handle that
		import re
		import json
		
		def fix_llm_json_mistakes(data: dict) -> dict:
			"""Fix common LLM mistakes in workflow JSON before validation."""
			# Fix 1: workflow_analysis should be a string, not an object
			if isinstance(data.get('workflow_analysis'), dict):
				# Convert the object to a formatted string
				wa = data['workflow_analysis']
				parts = []
			...
			if 'name' not in data:
				data['name'] = 'Generated Workflow'
			if 'description' not in data:
				data['description'] = 'Automatically generated workflow'
			if 'version' not in data:
				data['version'] = '1.0'
			
			# Fix 3: Fix steps with missing required fields
			if 'steps' in data:
				for i, step in enumerate(data['steps']):
					step_type = step.get('type')
					
					# Fix input steps missing 'value' field
					if step_type == 'input' and 'value' not in step:
						# Try to infer value from description or add placeholder
						desc = step.get('description', '')
						# Check if there's a variable reference we can extract
						var_match = re.search(r'\{\{(\w+)\}\}', desc)
						if var_match:
							step['value'] = f"{{{{{var_match.group(1)}}}}}"
						else:
							# Use a placeholder variable based on target_text
							target = step.get('target_text', 'input').lower().replace(' ', '_')
							step['value'] = f"{{{{{target}}}}}"
						print(f"  🔧 Fixed input step {i} missing 'value': added '{step['value']}'")
					
					# Fix key_press steps missing 'key' field
					if step_type == 'key_press' and 'key' not in step:
						# Default to Enter key if not specified
						step['key'] = 'Enter'
						print(f"  🔧 Fixed key_press step {i} missing 'key': defaulted to 'Enter'")
					
					# Fix scroll steps missing scrollX/scrollY
					if step_type == 'scroll':
						if 'scrollX' not in step:
							step['scrollX'] = 0
						if 'scrollY' not in step:
							step['scrollY'] = step.get('scroll_y', 500)  # Common typo
							print(f"  🔧 Fixed scroll step {i} missing 'scrollY'")
					
					# Fix select_change steps missing 'selectedText'
					if step_type == 'select_change' and 'selectedText' not in step:
						desc = step.get('description', 'option')
						step['selectedText'] = f"{{{{{desc.lower().replace(' ', '_')}}}}}"
						print(f"  🔧 Fixed select_change step {i} missing 'selectedText'")
					
					# Fix extract_page_content steps missing 'goal'
					if step_type == 'extract_page_content' and 'goal' not in step:
						step['goal'] = step.get('description', 'Extract page content')
						print(f"  🔧 Fixed extract_page_content step {i} missing 'goal'")
					
					# Fix agent steps missing 'task'
					if step_type == 'agent' and 'task' not in step:
						step['task'] = step.get('description', 'Complete the task')
						print(f"  🔧 Fixed agent step {i} missing 'task'")
			
			return data
		
		def try_parse_and_fix_json(json_text: str) -> WorkflowDefinitionSchema:
			"""Try to parse JSON and fix common LLM mistakes."""
			try:
				# First try direct parse
				return WorkflowDefinitionSchema.model_validate_json(json_text)
			except Exception as first_error:
				# Try to fix the JSON
				try:
					data = json.loads(json_text)
					fixed_data = fix_llm_json_mistakes(data)
					return WorkflowDefinitionSchema.model_validate(fixed_data)
				except Exception:
					raise first_error  # Re-raise original error if fix fails
		...
		try:
			...
			# Check if the error contains the raw response (some providers include it)
			# This avoids making a second LLM call
			workflow_definition = None
			
			# Try to extract JSON from the error message if it contains the response
			# Pattern: look for markdown-wrapped JSON in the error details
			if '```json' in error_str or '```\n{' in error_str:
				print('\n🔧 Detected markdown-wrapped JSON in error, attempting extraction...')
				json_match = re.search(r'```(?:json)?\s*([\s\S]*?)\s*```', error_str)
				if json_match:
					try:
						json_text = json_match.group(1)
						workflow_definition = try_parse_and_fix_json(json_text)
						print('✅ Successfully parsed workflow from markdown-wrapped JSON in error')
					except Exception as parse_err:
						print(f'Markdown extraction failed: {parse_err}')
			
			# If that didn't work, make a raw call without structured output format
			if workflow_definition is None:
				try:
					print('\n🔄 Retrying without structured output format...')
					raw_response = await self.llm.ainvoke(all_messages)
					raw_text = str(raw_response.completion) if hasattr(raw_response, 'completion') else str(raw_response)
					...
					if json_match:
						json_text = json_match.group(1)
						print('\n📝 Extracted JSON from markdown, attempting parse with fixes...')
						workflow_definition = try_parse_and_fix_json(json_text)
						print('✅ Successfully parsed workflow from markdown-wrapped JSON')
					else:
						# Try parsing the whole thing as JSON
						workflow_definition = try_parse_and_fix_json(raw_text)
						print('✅ Successfully parsed workflow from raw response')
				except Exception as parse_error:
					print(f'Manual parsing also failed: {parse_error}')
	...
	async def generate_workflow_from_prompt(
		...
		Args:
			...
			sensitive_data: Optional dict of credential name -> value (not logged)
		"""
...
This structured format is critical for generating a reusable workflow."""
		...
		# If sensitive data is provided, inject it into the agent's system message
		# This keeps credentials out of the task log while still making them available
		sensitive_system_prompt = None
		if sensitive_data:
			creds_text = '\n'.join(f'{k}={v}' for k, v in sensitive_data.items())
			sensitive_system_prompt = f"""CREDENTIALS (use these for login - DO NOT output these values in your reasoning):
{creds_text}

Use the appropriate credentials based on the website/service."""
		...
		agent = Agent(
			...
			extend_system_message=sensitive_system_prompt,
		)
