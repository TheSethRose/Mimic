...
class SemanticWorkflowExecutor:
	...
	async def _element_is_truly_visible_and_focusable(self, element) -> bool:
		"""Check if an element is truly visible, in viewport, and focusable.
		
		This is more thorough than _element_is_visible and checks:
		1. Element has a valid bounding box
		2. Element is in the viewport
		3. Element is not obscured by other elements
		4. Element passes offsetParent check (for visibility)
		"""
		try:
			...
			# Use JavaScript to check if element is truly visible and focusable
			result = await self._element_evaluate(
				element,
				'''(function() {
					// Check if element has an offsetParent (hidden elements don't)
					if (this.offsetParent === null && 
						getComputedStyle(this).position !== 'fixed' && 
						getComputedStyle(this).position !== 'sticky') {
					...
					if (style.visibility === 'hidden' || style.display === 'none' || style.opacity === '0') {
						return { visible: false, reason: 'css_hidden' };
					}
					...
					if (rect.top > window.innerHeight || rect.bottom < 0 ||
						...
						return { visible: false, reason: 'out_of_viewport' };
					}
					
					// Check if element is at center point (not obscured)
					const centerX = rect.left + rect.width / 2;
					const centerY = rect.top + rect.height / 2;
					const elementAtPoint = document.elementFromPoint(centerX, centerY);
					
					if (elementAtPoint) {
						// Element at point should be this element or a descendant
						const isVisible = (elementAtPoint === this || this.contains(elementAtPoint) || 
										  elementAtPoint.contains(this));
						if (!isVisible) {
							return { visible: false, reason: 'obscured' };
						}
					}
			...
			if result and result.get('visible'):
				...
				logger.debug(f"Element not truly visible: {result.get('reason') if result else 'unknown'}")
				return False
	...
	async def _get_first_visible_element(self, elements: list, prefer_last: bool = True):
		"""Get the first visible and focusable element from a list of elements.
		
		This is useful when there are multiple matching elements (e.g., duplicate inputs
		where one is in a hidden modal and one is visible).
		
		Args:
			elements: List of elements to check
			prefer_last: If True, check elements in reverse order (useful for modals/overlays
			             which are typically appended at the end of the DOM)
		"""
		# Check elements in reverse order if prefer_last is True
		# Modal overlays are typically appended at the end of the DOM
		elements_to_check = list(reversed(elements)) if prefer_last else elements
		
		# First pass: try to find truly visible and focusable element
		for element in elements_to_check:
			try:
				is_truly_visible = await self._element_is_truly_visible_and_focusable(element)
				if is_truly_visible:
					return element
			except Exception:
				continue
		
		# Second pass: fall back to basic visibility check
		for element in elements_to_check:
			try:
		...
		# Fallback to last element (for overlays) or first if prefer_last is False
		return elements[-1] if elements and prefer_last else (elements[0] if elements else None)
	...
	def _find_input_element_by_text(self, target_text: str) -> Optional[Dict]:
		"""Find INPUT element specifically by label text - prioritizes input/textarea elements.
		
		This is a specialized version of _find_element_by_text that:
		1. Only matches input, textarea, select elements
		2. Prioritizes label_text matches over text_content matches
		3. Avoids matching links (like "Forgot Password?") when looking for input fields
		"""
		if not target_text:
			return None
		...
		# Strategy 1: Exact label_text match on input elements (highest priority)
		for text, element_info in self.current_mapping.items():
			element_type = element_info.get('element_type', '').upper()
			if element_type not in ['INPUT', 'TEXTAREA', 'SELECT']:
				continue
			
			label_text = element_info.get('label_text', '').lower().strip()
			if label_text and label_text == target_lower:
				logger.info(f"Found input by exact label match: '{target_text}' -> {element_info['selectors']}")
				return element_info
		
		# Strategy 2: Label text contains target (for inputs only)
		for text, element_info in self.current_mapping.items():
			element_type = element_info.get('element_type', '').upper()
			if element_type not in ['INPUT', 'TEXTAREA', 'SELECT']:
				continue
			...
			if label_text and target_lower in label_text:
				logger.info(f"Found input by label contains match: '{target_text}' -> {element_info['selectors']}")
				return element_info
		
		# Strategy 3: Placeholder text match on inputs
		for text, element_info in self.current_mapping.items():
			element_type = element_info.get('element_type', '').upper()
			if element_type not in ['INPUT', 'TEXTAREA', 'SELECT']:
				continue
			
			placeholder = (element_info.get('placeholder') or '').lower().strip()
			if placeholder and target_lower in placeholder:
				logger.info(f"Found input by placeholder match: '{target_text}' -> {element_info['selectors']}")
				return element_info
		
		# Strategy 4: Name attribute contains target (common for form fields)
		for text, element_info in self.current_mapping.items():
			element_type = element_info.get('element_type', '').upper()
			if element_type not in ['INPUT', 'TEXTAREA', 'SELECT']:
				continue
			
			selector = element_info.get('selectors', '').lower()
			# Check if name= attribute contains target
			if f'name="{target_lower}"' in selector or f"[name*='{target_lower}']" in selector:
				logger.info(f"Found input by name attribute: '{target_text}' -> {element_info['selectors']}")
				return element_info
			
			# Also check name without quotes for partial matches
			if 'name=' in selector:
				name_match = selector.split('name=')[1].split(']')[0].replace('"', '').replace("'", '').lower()
				if target_lower in name_match or name_match in target_lower:
					logger.info(f"Found input by name partial match: '{target_text}' -> {element_info['selectors']}")
					return element_info
		
		# Strategy 5: Fall back to general _find_element_by_text but only return if it's an input
		general_match = self._find_element_by_text(target_text)
		if general_match:
			element_type = general_match.get('element_type', '').upper()
			if element_type in ['INPUT', 'TEXTAREA', 'SELECT']:
				return general_match
			else:
				logger.warning(f"General match for '{target_text}' found non-input element: {element_type}")
	...
	async def execute_input_step(self, step: InputStep) -> ActionResult:
		...
		if hasattr(step, 'target_text') and step.target_text:
			...
			# If direct selector fails, try specialized INPUT element finder first
			if not selector_to_use:
				# Use specialized input finder that prioritizes input/textarea elements
				element_info = self._find_input_element_by_text(step.target_text)
				if element_info:
					selector_to_use = element_info['selectors']
					logger.info(f"Using semantic mapping (input-specific): '{target_identifier}' -> {selector_to_use}")
				else:
					# Fall back to general element finder
					element_info = self._find_element_by_text(step.target_text)
					if element_info:
						selector_to_use = element_info['selectors']
						logger.info(f"Using semantic mapping: '{target_identifier}' -> {selector_to_use}")
			...
			target_identifier = step.description
			# Use specialized input finder first
			element_info = self._find_input_element_by_text(step.description)
			if element_info:
				selector_to_use = element_info['selectors']
				logger.info(f"Using semantic mapping (input-specific): '{target_identifier}' -> {selector_to_use}")
			else:
		...
		# Handle case where element_type is None or doesn't have expected keys
		if element_type is None:
			element_type = {'tagName': 'INPUT', 'type': 'text', 'value': ''}
			logger.warning(f'Could not determine element type for {selector_to_use}, assuming text input')
		
		if element_type.get('tagName') == 'SELECT':
			return ActionResult(
		...
		async def input_executor():
			...
			# When multiple elements match, try each one until input succeeds
			# This handles cases like duplicate inputs (one hidden, one visible in modal)
			# Start from the last element (modals are typically appended at the end of DOM)
			elements_to_try = list(reversed(elements)) if len(elements) > 1 else elements
			if len(elements) > 1:
				logger.info(f'Found {len(elements)} matching elements, will try each (last first) until input succeeds')
			
			last_error = None
			successful_element_idx = None
			for idx, element in enumerate(elements_to_try):
				try:
					element_type = await self._element_evaluate(
						element, '(function() { return { tagName: this.tagName, type: this.type, value: this.value }; })'
					)
					
					# Handle case where element_type is None
					if element_type is None:
						element_type = {'tagName': 'INPUT', 'type': 'text', 'value': ''}
						logger.debug(f'Could not determine element type for element {idx}, assuming text input')

					# Handle radio buttons and checkboxes with improved strategies
					if element_type.get('type') in ['radio', 'checkbox']:
					...
					# First, check if element is focusable by trying to click it
					try:
						# Try to focus/click the element first to check if it's interactive
						await element.click()
						await asyncio.sleep(0.1)
					except Exception as click_error:
						# Element not clickable/focusable - skip to next
						logger.debug(f'Element {idx} not focusable, trying next: {click_error}')
						if idx < len(elements_to_try) - 1:
							continue
						# Last element - still try to fill
					
					# Try to clear and fill the element
					try:
						...
						logger.debug(f'Element {idx} fill failed: {fill_error}')
						if idx < len(elements_to_try) - 1:
							continue  # Try next element
						raise
					
					# Verify the input was successful by checking the element's value
					try:
						current_value = await self._element_get_property(element, 'value')
						if current_value and step.value.lower() in str(current_value).lower():
							successful_element_idx = idx
							msg = f"⌨️ Input '{step.value}' into: {target_identifier or step.description or selector_to_use}"
							logger.info(msg)
							return ActionResult(extracted_content=msg, include_in_memory=True)
						else:
							# Input didn't stick - try next element
							logger.info(f'Element {idx} input value mismatch: expected "{step.value}", got "{current_value}" - trying next')
							if idx < len(elements_to_try) - 1:
								continue
					except Exception as verify_error:
						logger.debug(f'Element {idx} value verification failed: {verify_error}')
						if idx < len(elements_to_try) - 1:
							continue
					
					# If we get here on the last element, return success anyway
					# (verification will be done by the outer retry loop)
					msg = f"⌨️ Input '{step.value}' into: {target_identifier or step.description or selector_to_use}"
					logger.info(msg)
					return ActionResult(extracted_content=msg, include_in_memory=True)
					...
					last_error = e
					if idx < len(elements_to_try) - 1:
						logger.debug(f'Element {idx} failed, trying next: {e}')
						continue
					raise
			
			if last_error:
				raise last_error
			raise Exception(f'Failed to input into any of {len(elements_to_try)} matching elements')
		...
		async def input_verifier():
			...
			# When multiple elements match, check ALL of them for the correct value
			# Return True if ANY element has the correct value
			for idx, element in enumerate(reversed(elements)):  # Check last first (modal element)
				try:
					current_value = await self._element_get_property(element, 'value')
					if current_value and step.value.lower() in str(current_value).lower():
						logger.info(f'Verification: Input value found in element {idx}: "{current_value}"')
						return True
				except Exception:
					continue
			
			# None of the elements have the correct value
			logger.info(f'Verification: Input expected "{step.value}", not found in any of {len(elements)} elements')
			return False
			...
			# For buttons, verify the click had some effect
				...
				try:
					...
					if button_exists and element:
						...
						# If button is no longer visible, the click likely triggered a state change
						# (e.g., modal opened, page transition, form submitted). This is success!
						if not is_visible:
							logger.info(f"Verification: Button '{target_text}' became hidden after click - likely successful state change")
							return True
